...
const sizeStyles: { [s in size]: string } = {
    small: styles.small
};

export function Spinner({ size }: SpinnerProps) {
    const className = cx(styles.Spinner, size && sizeStyles[size]);

    const spinnerSVGMarkup = (
        <svg className="animate-spin" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            <g>
                <path
